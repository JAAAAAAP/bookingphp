...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["group"])) {
        ...
        $groupid_sql = 'SELECT o.*, u.user, p.name AS products_name, p.sn_products,p.amount AS products_amount 
                        ...
                        INNER JOIN USER AS u ON u.user_id = o.user_id WHERE group_id = :g_id';
...
foreach ($groupid as $group) {
    ...
    switch ($group['products_name']) {
        ...
        case 'สาย VGA':
            ...
            break;
        case 'เครื่องฉายแผ่นทึบ':
            $Opaque_projector = "<div style='position: absolute; top: 224px; left: 390px;font-size:13;'><h1>X</h1></div>";
            break;
        case 'ลำโพง/เครื่องเสียง':
            $Audio = "<div style='position: absolute; top: 248px; left: 390px;font-size:13;'><h1>X</h1></div>";
            break;
        case 'ไมค์ลอย':
...
$html = $user . $tel . $department . $address . $teacher . $date_start . $date_end . $checked1 . $checked2 . $projecter . $Notebook1 . $VGA . $Notebook2
    . $MacBook . $HDMI_to_VGA . $Opaque_projector . $Audio . $wireless_microphone . $wired_microphone . $audio_cable . $Extension_plug;
$mpdf->WriteHTML($html);
$mpdf->Output();
